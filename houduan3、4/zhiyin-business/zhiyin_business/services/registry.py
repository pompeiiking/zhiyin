...
落位：`business/services/registry.py` —— 后端-4。
...
实现口径：方法体是一行转发（`await self._registry.xxx()`）。上下线与排序口径
由 Repository 统一负责（见 `RegistryRepository` 的 docstring），**这里不再写一遍
status 过滤与 sort**。
"""
