...
| 当前状态 | OPEN-1、OPEN-2、OPEN-3、OPEN-4、OPEN-5 已闭合；OPEN-6 已修（见第五节）；MOCK-1、MOCK-2 于 2026-09-19 因「清除 Mock」变更失去对象（见第五节）；**MOCK-3（检索内容仍是演示卡）于 2026-09-19 新发现，未闭合（见 §五之后）** |
...
### MOCK-3 · **检索内容本身仍是演示卡**，且被门禁算作已接线 —— 未闭合（2026-09-19 追加）
...
| 影响 | 这 6 张演示卡会进入证据包，并**可能被报告当引用**（引用核对只校验"引用是否出现在证据包里"，不校验证据本身是不是演示数据） |
| 归属 | 平台侧（RAG 知识库解析被 `callback_url` 阻塞）+ 内容侧（理论卡/来源台账待定稿） |
| 退出判据 | 向量库与权威文档表有真实内容，或明确改为"演示内容"在装配报告与响应里**可识别**（二选一，需记录决策） |
| 决策登记 | [待决问题 D12](职引-待决问题与改法选项-v1.0.md)（A 硬失败 + 显式 opt-in / B 证据打演示标记 / C 只记录；建议先做"如实上报"再打标，**不单独做 C**） |

证据与复现命令见[修改日志 §20](../00-索引与变更/职引-数据全链路修改日志.md)与[检索质量基准 §六](职引-检索质量基准-2026-09-19.md)。
