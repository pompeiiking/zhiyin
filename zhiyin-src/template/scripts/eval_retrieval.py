...
def corpus_ids(knowledge_dir: Path) -> dict[str, set[str]]:
    """本地语料里每个 namespace 的文档 id（裸 id 与 `namespace:id` 两种写法都收集）。

    同时收集两种形态是刻意的：评测集的期望写成 `namespace:id`，而本地关键词通道
    当前返回**裸 id**——把两种都算出来，报告才能指出"到底是没这条内容，还是
    两边的 id 口径不一致"。
    """
...
def coverage_report(dataset: Path, ids: dict[str, set[str]]) -> dict[str, Any]:
    ...
    qualified_expected = bare_corpus = 0
    for case in cases:
        ...
        raw_ids = {item for item in present if ":" not in item}
        if raw_ids:
            bare_corpus += 1
    return {
        ...
        # 诊断用：两边 id 口径是否一致。实验证明它们**不一致**——
        # 评测集的期望写成 `namespace:id`，而 `LocalSearchGateway` 返回裸 `id`。
        "expectations_namespace_qualified": qualified_expected == len(cases) and bool(cases),
        "corpus_ids_are_bare": bare_corpus > 0,
    }
...
def main() -> int:
    ...
    if coverage["expectations_namespace_qualified"] and coverage["corpus_ids_are_bare"]:
        print(
            "  id 口径不一致: 评测集期望写 `namespace:id`，而本地关键词通道返回裸 id。\n"
            "    这会让「内容明明存在却匹配不上」，也会让关键词通道与向量通道的同一篇"
            "文档在 RRF 融合时被当成两篇（去重键是 evidence_id）。已登记待修。"
        )
