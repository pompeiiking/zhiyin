...
服务落位表（对应《第一期技术架构文档》§4.5.1 模块归属）。第一期服务均已按
Port 实现并进入装配表，`IMPLEMENTATION_STATUS = "wired"`。

| 服务 | 文件 | 类 | 依赖 | 优先级 | 状态 |
| --- | --- | --- | --- | --- | --- |
| Loop 协调器 | `loop.py` | `AgentDrivenLoopCoordinator` | AgentEngine、TaskSessionRepository | P0 | 已实现 |
| Orchestrator | `orchestrator.py` | `DefaultOrchestrator` | 黑板四件套 + `policies/` + AgentEngine | P0 | 已实现 |
| Profile 服务 | `profile.py` | `DefaultProfileService` | ProfileRepository、EventBus | P0 | 已实现 |
| Behavior 服务 | `behavior.py` | `DefaultBehaviorService` | BehaviorRepository、EventBus | P0 | 已实现 |
| Memory 服务 | `memory.py` | `DefaultConversationMemoryService` | ConversationMemoryRepository | P0 | 已实现 |
| Asset 服务 | `asset.py` | `DefaultAssetService` | AssetRepository、EventBus + `policies/impact.py` | P0 | 已实现 |
| Workspace 服务 | `workspace.py` | `DefaultWorkspaceService` | 读侧聚合（Profile / Asset / Memory） | P1 | 已实现 |
| Function 服务 | `function.py` | `DefaultFunctionService` | ObjectStore、日历 | P1 | 已实现 |
| Identity 服务 | `identity.py` | `DefaultIdentityService` | AuthGateway、UserRepository | P0 | 已实现 |
| Registry 服务 | `registry.py` | `DefaultRegistryService` | RegistryRepository、FeatureFlagGateway | P0 | 已实现 |
...
实现和装配结果由 `tests/e2e/test_main_path.py` 与 `--check --phase=2` 持续守卫。
"""
